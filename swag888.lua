...
RunBtn1.Parent.Visible = WhitelistedUserIDs[LocalPlayer.UserId] == true
...
local AntiSitConn
local AntiSitStateConn
local AntiSitLoopConn
local NoclipConn
local AntiFlingConn
local AntiEventConn
local AntiEventDescConn
local AntiLagRunning = false

local function ForceAntiSit(hum)
    if not hum or not hum.Parent or not ProtectionState.AntiSit then return end

    -- ปิดสถานะ Seated โดยตรง และบังคับคืนสถานะทันที
    pcall(function()
    ...
    if hum:GetState() == Enum.HumanoidStateType.Seated then
        ...
        task.defer(function()
            if hum.Parent and ProtectionState.AntiSit then
                hum.Sit = false
...
local function SetupAntiSit(state)
    ...
    if AntiSitStateConn then AntiSitStateConn:Disconnect(); AntiSitStateConn = nil end
    if AntiSitLoopConn then AntiSitLoopConn:Disconnect(); AntiSitLoopConn = nil end
    ...
    local function hookCharacter(character)
        ...
        if not humanoid or not ProtectionState.AntiSit then return end

        pcall(function()
            humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated, false)
        end)
        ForceAntiSit(humanoid)

        AntiSitConn = humanoid.Seated:Connect(function(active)
            if active and ProtectionState.AntiSit then
                ForceAntiSit(humanoid)
            end
        end)

        AntiSitStateConn = humanoid.StateChanged:Connect(function(_, newState)
            if ProtectionState.AntiSit and newState == Enum.HumanoidStateType.Seated then
                ForceAntiSit(humanoid)
            end
        end)

        AntiSitLoopConn = RunService.Heartbeat:Connect(function()
            if ProtectionState.AntiSit and humanoid.Parent then
                -- ตรวจทุกเฟรมเพื่อไม่ให้การบังคับนั่งค้างแม้เพียงชั่วขณะ
                if humanoid.Sit
...
LocalPlayer.CharacterAdded:Connect(function(char)
    task.wait(0.1)
    if ProtectionState.AntiSit then
        SetupAntiSit(true)
    end
    if ProtectionState.AntiEvent then
        task.wait()
        -- AntiEvent จะผูกกับตัวละครใหม่ผ่าน SetupAntiEvent
        SetupAntiEvent(true)
    end
end)
...
local function SetupAntiFling(state)
    ...
    AntiFlingConn = RunService.Heartbeat:Connect(function()
        ...
        if root then
            ...
            -- กันทั้งแรงส่งตรงและแรงหมุนจากการชน/ฟิสิกส์ผิดปกติ
            if linear.Magnitude > 65 then
                root.AssemblyLinearVelocity = Vector3.zero
            end
            if angular.Magnitude > 65 then
                root.AssemblyAngularVelocity = Vector3.zero
            end

            -- ถ้าตัวละครถูกส่งลง/ขึ้นแบบผิดปกติ ให้ตัดความเร็วทันที
            if math.abs(linear.Y) > 65 then
                root.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
            end
        end

        -- ป้องกันแรง/ความเร็วที่ถูกใส่ลงส่วนอื่นของตัวละคร
        for _, obj in ipairs(char:GetDescendants()) do
            if obj:IsA("BasePart") then
                if obj.AssemblyLinearVelocity.Magnitude > 65 then
                    obj.AssemblyLinearVelocity = Vector3.zero
                end
                if obj.AssemblyAngularVelocity.Magnitude > 65 then
        ...
        if hum then
            -- ไม่ปล่อยให้สถานะฟิสิกส์จากการโดนดีดค้างนาน
            local current = hum:GetState()
            if current == Enum.HumanoidStateType.FallingDown
                or current == Enum.HumanoidStateType.Ragdoll then
                hum:ChangeState(Enum.HumanoidStateType.GettingUp)
            end
        end
    end)
...
-- =====================================================================
-- 🧹 ANTI EVENT
-- ป้องกันผลกระทบจากของ Event ที่มากระทำกับตัวละคร เช่น
-- กล้วย/พื้นลื่น, แทรมโพลีน และปืนตด โดยล้างเอฟเฟกต์/แรงที่ถูกใส่กับตัวละคร
-- =====================================================================

local AntiEventNames = {
    "banana", "slip", "slippery", "trampoline",
    "fart", "stink", "gas", "smell", "event"
}

local function IsAntiEventObject(obj)
    local name = string.lower(obj.Name or "")
    for _, key in ipairs(AntiEventNames) do
...
local function CleanEventEffects(character)
    if not character or not ProtectionState.AntiEvent then return end

    -- ลบวัตถุเอฟเฟกต์ของ Event ที่ถูกใส่เข้าตัวละคร
    for _, obj in ipairs(character:GetDescendants()) do
        if IsAntiEventObject(obj) then
            pcall(function()
                obj:Destroy()
            end)
        end
    ...
    -- ล้างแรง/ตัวควบคุมฟิสิกส์ที่ Event ใช้ผลัก/ดีด/ทำให้เด้ง
    for _, obj in ipairs(character:GetDescendants()) do
    ...
    if hum then
        -- คืนสถานะที่ Event มักใช้ทำให้ตัวละครเสียการควบคุม
        hum.Sit = false
        hum.PlatformStand = false

        local current = hum:GetState()
        if current == Enum.HumanoidStateType.Ragdoll
    ...
    if root then
        -- ตัดแรงเด้ง/แรงส่งจาก Event โดยตรง
        if root.AssemblyLinearVelocity.Magnitude > 55 then
            root.AssemblyLinearVelocity = Vector3.zero
        end
        if root.AssemblyAngularVelocity.Magnitude > 55 then
            root.AssemblyAngularVelocity = Vector3.zero
        end
    end
end

function SetupAntiEvent(state)
    if AntiEventConn then AntiEventConn:Disconnect(); AntiEventConn = nil end
    if AntiEventDescConn then AntiEventDescConn:Disconnect(); AntiEventDescConn = nil end

    ProtectionState.AntiEvent = state
    if not state then return end
    ...
    local function hookCharacter(character)
        ...
        CleanEventEffects(character)

        -- ถ้ามีของ Event ถูกใส่เข้ามาทีหลัง ให้ลบทันที
        AntiEventDescConn = character.DescendantAdded:Connect(function(obj)
            if not ProtectionState.AntiEvent then return end
            task.defer(function()
                if ProtectionState.AntiEvent and obj.Parent then
                    if IsAntiEventObject(obj)
    ...
    AntiEventConn = RunService.Heartbeat:Connect(function()
        if ProtectionState.AntiEvent then
            local currentChar = LocalPlayer.Character
            if currentChar then
                CleanEventEffects(currentChar)
            end
        end
    end)
end
...
CreateSwitchCard(PageProtection, "3. anti fling", "ป้องกันแรงดีด/แรงเหวี่ยง/แรงหมุนผิดปกติ", SetupAntiFling)
CreateSwitchCard(PageProtection, "4. ป้องกันแลค-กระตุก", "ตัวช่วยเก็บหน่วยความจำเป็นระยะ โดยไม่ลบวัตถุหรือปิดเงา", SetupAntiLag)
CreateSwitchCard(PageProtection, "5. กันของอีเว้นท์", "ป้องกันผลกระทบจากของ Event เช่น กล้วยลื่น แทรมโพลีน และปืนตด", SetupAntiEvent)
