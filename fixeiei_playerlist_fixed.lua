...
-- ==================== AUTHORIZED USER ID ====================
-- ใส่ UserId ที่อนุญาตให้ใช้สคริปต์ได้ที่นี่ เช่น [123456789] = true
...
if not AuthorizedUserIds[LocalPlayer.UserId] then
    pcall(function()
        LocalPlayer:Kick("ซื้อสคริปผมหน่อยยpls")
    end)
    return
end
...
-- ==================== PREMIUM PURPLE/BLACK FRAME ====================
local AuraGlow = Instance.new("Frame", ScreenGui)
AuraGlow.Name = "AuraGlow"
AuraGlow.Size = UDim2.new(0.95, 0, 0.84, 0)
AuraGlow.Position = UDim2.new(0.5, 0, 0.5, 0)
AuraGlow.AnchorPoint = Vector2.new(0.5, 0.5)
AuraGlow.BackgroundColor3 = Color3.fromRGB(130, 40, 255)
AuraGlow.BackgroundTransparency = 0.78
AuraGlow.BorderSizePixel = 0
AuraGlow.ZIndex = 1
Instance.new("UICorner", AuraGlow).CornerRadius = UDim.new(0, 22)

local AuraGradient = Instance.new("UIGradient", AuraGlow)
AuraGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(75, 15, 150)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(180, 55, 255)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(55, 10, 120))
}
AuraGradient.Rotation = 35

local Shadow3D = Instance.new("Frame", ScreenGui)
Shadow3D.Name = "Shadow3D"
Shadow3D.Size = UDim2.new(0.91, 0, 0.80, 0)
Shadow3D.Position = UDim2.new(0.5, 8, 0.5, 9)
Shadow3D.AnchorPoint = Vector2.new(0.5, 0.5)
Shadow3D.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
Shadow3D.BackgroundTransparency = 0.18
Shadow3D.BorderSizePixel = 0
Shadow3D.ZIndex = 2
Instance.new("UICorner", Shadow3D).CornerRadius = UDim.new(0, 20)

local LEDBorder = Instance.new("Frame", ScreenGui)
LEDBorder.Name = "LEDBorder"
LEDBorder.Size = UDim2.new(0.91, 0, 0.80, 0)
LEDBorder.Position = UDim2.new(0.5, 0, 0.5, 0)
LEDBorder.AnchorPoint = Vector2.new(0.5, 0.5)
LEDBorder.BackgroundColor3 = Color3.fromRGB(115, 25, 220)
LEDBorder.BorderSizePixel = 0
LEDBorder.Active = true
LEDBorder.ZIndex = 3
Instance.new("UICorner", LEDBorder).CornerRadius = UDim.new(0, 20)

local LEDGradient = Instance.new("UIGradient", LEDBorder)
LEDGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(95, 20, 210)),
    ColorSequenceKeypoint.new(0.28, Color3.fromRGB(245, 120, 255)),
    ColorSequenceKeypoint.new(0.52, Color3.fromRGB(95, 20, 210)),
    ColorSequenceKeypoint.new(0.78, Color3.fromRGB(40, 5, 90)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(175, 45, 255))
}
...
MainGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(13, 8, 23)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(8, 6, 14)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(18, 7, 29))
}
MainGradient.Rotation = 90

-- Soft inner glow
local InnerStroke = Instance.new("UIStroke", MainFrame)
InnerStroke.Color = Color3.fromRGB(165, 65, 255)
InnerStroke.Transparency = 0.55
InnerStroke.Thickness = 1.2

-- ==================== MOVING LED RUNNER ====================
local LEDRunner = Instance.new("Frame", LEDBorder)
LEDRunner.Name = "LEDRunner"
LEDRunner.Size = UDim2.new(0, 72, 0, 4)
LEDRunner.AnchorPoint = Vector2.new(0.5, 0.5)
LEDRunner.Position = UDim2.new(0, 10, 0, 3)
LEDRunner.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
LEDRunner.BorderSizePixel = 0
LEDRunner.ZIndex = 20
Instance.new("UICorner", LEDRunner).CornerRadius = UDim.new(1, 0)

local RunnerGlow = Instance.new("UIStroke", LEDRunner)
RunnerGlow.Color = Color3.fromRGB(225, 100, 255)
RunnerGlow.Thickness = 5
RunnerGlow.Transparency = 0.45

local RunnerGradient = Instance.new("UIGradient", LEDRunner)
RunnerGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(80, 10, 160)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 210, 255)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(80, 10, 160))
}
...
ParticleCanvas.ZIndex = 5

for i = 1, 22 do
    local Star = Instance.new("Frame", ParticleCanvas)
    Star.Size = UDim2.new(0, math.random(2, 4), 0, math.random(2, 4))
    Star.Position = UDim2.new(math.random(), 0, math.random(), 0)
    Star.BackgroundColor3 = (i % 3 == 0) and Color3.fromRGB(220, 130, 255) or Color3.fromRGB(125, 70, 220)
    Star.BorderSizePixel = 0
    Star.BackgroundTransparency = math.random(5, 9) / 10
    Star.ZIndex = 5
    Instance.new("UICorner", Star).CornerRadius = UDim.new(1, 0)

    task.spawn(function()
        while Star and Star.Parent do
            local targetY = Star.Position.Y.Scale - 0.16
            if targetY < -0.1 then targetY = 1.1 end
            local tween = TweenService:Create(Star, TweenInfo.new(math.random(15, 28) / 10, Enum.EasingStyle.Linear), {
...
local function updateMovingLED()
    if not LEDBorder.Visible then return end
    local size = LEDBorder.AbsoluteSize
    local w, h = size.X, size.Y
    if w <= 0 or h <= 0 then return end

    local t = (tick() * 135) % (2 * (w + h - 16))
    local x, y
    local margin = 7
    local top = margin
    local right = w - margin
    local bottom = h - margin
    local perimeter = 2 * ((right - margin) + (bottom - margin))

    if t < (right - margin) then
        x, y = margin + t, top
    elseif t < (right - margin) + (bottom - top) then
        x, y = right, top + (t - (right - margin))
    elseif t < 2 * (right - margin) + (bottom - top) then
        x, y = right - (t - ((right - margin) + (bottom - top))), bottom
    else
        x, y = margin, bottom - (t - (2 * (right - margin) + (bottom - top)))
    end

    LEDRunner.Position = UDim2.fromOffset(x, y)
    if math.abs(y - top) < 2 or math.abs(y - bottom) < 2 then
        LEDRunner.Size = UDim2.fromOffset(72, 4)
    else
        LEDRunner.Size = UDim2.fromOffset(4, 72)
    end
end

RunService.RenderStepped:Connect(function()
    if LEDBorder.Visible then
        LEDGradient.Rotation = (LEDGradient.Rotation + 1.3) % 360
        AuraGradient.Rotation = (AuraGradient.Rotation + 0.25) % 360
        updateMovingLED()
        AuraGlow.BackgroundTransparency = 0.70 + ((math.sin(tick() * 2.5) + 1) / 2 * 0.14)
    end
end)
...
-- ==================== LINE ICON DRAWING ====================
local function iconLine(parent, x, y, w, h, rot)
    local line = Instance.new("Frame", parent)
    line.AnchorPoint = Vector2.new(0.5, 0.5)
    line.Position = UDim2.new(0, x, 0, y)
    line.Size = UDim2.new(0, w, 0, h)
    line.BackgroundColor3 = Color3.fromRGB(215, 160, 255)
    line.BorderSizePixel = 0
    line.Rotation = rot or 0
    line.ZIndex = (parent.ZIndex or 1) + 3
    Instance.new("UICorner", line).CornerRadius = UDim.new(1, 0)
    return line
end

local function makeLineIcon(parent, kind)
    local box = Instance.new("Frame", parent)
    box.Size = UDim2.new(0, 24, 0, 24)
    box.Position = UDim2.new(0, 7, 0.5, -12)
    box.BackgroundTransparency = 1
    box.ZIndex = (parent.ZIndex or 1) + 3

    local function L(x, y, w, h, r)
        return iconLine(box, x, y, w, h, r)
    end

    if kind == "bolt" then
        L(13, 5, 3, 10, 25); L(10, 10, 3, 12, -25); L(8, 15, 3, 10, 25)
    elseif kind == "music" then
        L(15, 7, 2, 11, 0); L(19, 5, 2, 11, 0); L(17, 3, 8, 2, 0); L(8, 17, 6, 2, 0); L(16, 15, 6, 2, 0)
        local c1 = Instance.new("Frame", box); c1.Size = UDim2.fromOffset(7,5); c1.Position = UDim2.fromOffset(5,16); c1.BackgroundTransparency=1; c1.ZIndex=box.ZIndex
        local s1=Instance.new("UIStroke",c1); s1.Color=Color3.fromRGB(215,160,255); s1.Thickness=2
        Instance.new("UICorner",c1).CornerRadius=UDim.new(1,0)
    elseif kind == "headphones" then
        local arc = Instance.new("Frame", box); arc.Size=UDim2.fromOffset(18,16); arc.Position=UDim2.fromOffset(3,4); arc.BackgroundTransparency=1; arc.ZIndex=box.ZIndex
        local s=Instance.new("UIStroke",arc); s.Color=Color3.fromRGB(215,160,255); s.Thickness=2
        Instance.new("UICorner",arc).CornerRadius=UDim.new(0.5,0)
        L(5, 17, 3, 6, 0); L(19, 17, 3, 6, 0)
    elseif kind == "eye" then
        local eye = Instance.new("Frame", box); eye.Size=UDim2.fromOffset(17,11); eye.Position=UDim2.fromOffset(3,7); eye.BackgroundTransparency=1; eye.Rotation=45; eye.ZIndex=box.ZIndex
        local s=Instance.new("UIStroke",eye); s.Color=Color3.fromRGB(215,160,255); s.Thickness=1.8
        Instance.new("UICorner",eye).CornerRadius=UDim.new(0.5,0)
        local pupil=Instance.new("Frame",box); pupil.Size=UDim2.fromOffset(5,5); pupil.Position=UDim2.fromOffset(9,9); pupil.BackgroundColor3=Color3.fromRGB(215,160,255); pupil.BorderSizePixel=0; pupil.ZIndex=box.ZIndex
        Instance.new("UICorner",pupil).CornerRadius=UDim.new(1,0)
    elseif kind == "search" then
        local c=Instance.new("Frame",box); c.Size=UDim2.fromOffset(12,12); c.Position=UDim2.fromOffset(4,4); c.BackgroundTransparency=1; c.ZIndex=box.ZIndex
        local s=Instance.new("UIStroke",c); s.Color=Color3.fromRGB(215,160,255); s.Thickness=2; Instance.new("UICorner",c).CornerRadius=UDim.new(1,0)
        L(17,17,9,2,45)
    elseif kind == "clipboard" then
        local c=Instance.new("Frame",box); c.Size=UDim2.fromOffset(13,16); c.Position=UDim2.fromOffset(5,5); c.BackgroundTransparency=1; c.ZIndex=box.ZIndex
        local s=Instance.new("UIStroke",c); s.Color=Color3.fromRGB(215,160,255); s.Thickness=1.8; Instance.new("UICorner",c).CornerRadius=UDim.new(0,2)
        L(12,5,7,2,0); L(9,11,6,2,0); L(9,15,6,2,0)
    elseif kind == "tag" then
        L(7, 8, 10, 2, 45); L(12, 5, 8, 2, 0); L(17, 10, 8, 2, 90); L(9, 15, 10, 2, 45)
    elseif kind == "close" then
        L(12, 12, 16, 2, 45); L(12, 12, 16, 2, -45)
    elseif kind == "back" then
        L(9, 12, 13, 2, 0); L(7, 12, 8, 2, 45); L(7, 12, 8, 2, -45)
    end
    return box
end
...
TopBar.BackgroundColor3 = Color3.fromRGB(16, 10, 27)
...
TopBarLine.BackgroundColor3 = Color3.fromRGB(150, 50, 255)
TopBarLine.BorderSizePixel = 0
TopBarLine.ZIndex = 8

local Title = Instance.new("TextLabel", TopBar)
Title.Size = UDim2.new(0.53, 0, 1, 0)
...
TagToggleSwitch.Size = UDim2.new(0, 84, 0, 28)
TagToggleSwitch.Position = UDim2.new(1, -126, 0.5, -14)
TagToggleSwitch.BackgroundColor3 = Color3.fromRGB(95, 25, 150)
TagToggleSwitch.Text = "TAG  ON"
TagToggleSwitch.Font = Enum.Font.GothamBold
TagToggleSwitch.TextSize = 9
TagToggleSwitch.TextColor3 = Color3.fromRGB(255, 255, 255)
TagToggleSwitch.ZIndex = 9
Instance.new("UICorner", TagToggleSwitch).CornerRadius = UDim.new(0, 9)
local TagStroke = Instance.new("UIStroke", TagToggleSwitch)
TagStroke.Color = Color3.fromRGB(190, 85, 255)
TagStroke.Thickness = 1

makeLineIcon(TagToggleSwitch, "tag")

TagToggleSwitch.MouseButton1Click:Connect(function()
    ShowTagsEnabled = not ShowTagsEnabled
    TagToggleSwitch.Text = ShowTagsEnabled and "TAG  ON" or "TAG  OFF"
    TagToggleSwitch.BackgroundColor3 = ShowTagsEnabled and Color3.fromRGB(95, 25, 150) or Color3.fromRGB(75, 20, 45)
...
CloseBtn.BackgroundColor3 = Color3.fromRGB(55, 15, 75)
CloseBtn.Text = ""
CloseBtn.ZIndex = 10
Instance.new("UICorner", CloseBtn).CornerRadius = UDim.new(0, 10)
local CloseStroke = Instance.new("UIStroke", CloseBtn)
CloseStroke.Color = Color3.fromRGB(215, 90, 255)
CloseStroke.Thickness = 1.2
makeLineIcon(CloseBtn, "close")
...
ListScroll.BackgroundColor3 = Color3.fromRGB(12, 9, 20)
ListScroll.BorderSizePixel = 0
ListScroll.ScrollBarThickness = 4
ListScroll.ScrollBarImageColor3 = Color3.fromRGB(175, 70, 255)
ListScroll.ZIndex = 7
Instance.new("UICorner", ListScroll).CornerRadius = UDim.new(0, 11)
local ListStroke = Instance.new("UIStroke", ListScroll)
ListStroke.Color = Color3.fromRGB(75, 35, 110)
...
ListTitle.Size = UDim2.new(0.44, 0, 0, 24)
ListTitle.Position = UDim2.new(0.03, 0, 0.14, 0)
ListTitle.BackgroundTransparency = 1
ListTitle.Text = "PLAYERS"
ListTitle.TextColor3 = Color3.fromRGB(180, 110, 255)
...
local function create3DButton(parent, text, color, iconKind)
    ...
    stroke.Color = Color3.fromRGB(135, 65, 190)
    stroke.Transparency = 0.25
    stroke.Thickness = 1
    makeLineIcon(btn, iconKind)
    return btn
end

local GetIDBtn = create3DButton(ButtonsContainer, "เจาะไอดีเพลง", Color3.fromRGB(48, 18, 70), "bolt")
local GetJunkBtn = create3DButton(ButtonsContainer, "ยิงเปิดเพลงตาม player", Color3.fromRGB(38, 16, 58), "music")
local ListenToggleBtn = create3DButton(ButtonsContainer, "ฟังเพลงส่วนตัว 80%", Color3.fromRGB(24, 20, 62), "headphones")
local ViewRawJunkBtn = create3DButton(ButtonsContainer, "ดูข้อมูล player", Color3.fromRGB(24, 18, 36), "eye")
local ViewInstantBtn = create3DButton(ButtonsContainer, "ดู ID ที่เจาะมา", Color3.fromRGB(27, 18, 43), "search")
...
StatusLabel.BackgroundColor3 = Color3.fromRGB(13, 9, 22)
StatusLabel.Text = "เลือกชื่อผู้เล่นจากรายการด้านซ้าย"
StatusLabel.TextColor3 = Color3.fromRGB(190, 125, 255)
StatusLabel.Font = Enum.Font.GothamMedium
StatusLabel.TextSize = 9
StatusLabel.ZIndex = 8
Instance.new("UICorner", StatusLabel).CornerRadius = UDim.new(0, 8)
local StatusStroke = Instance.new("UIStroke", StatusLabel)
StatusStroke.Color = Color3.fromRGB(65, 30, 95)
StatusStroke.Thickness = 1

-- ==================== SQUARE OPEN/CLOSE BUTTON ====================
local ToggleBtn = Instance.new("ImageButton", ScreenGui)
ToggleBtn.Name = "HonkukiToggleButton"
ToggleBtn.Size = UDim2.new(0, 54, 0, 54)
ToggleBtn.Position = UDim2.new(0.02, 0, 0.45, 0)
ToggleBtn.BackgroundColor3 = Color3.fromRGB(17, 9, 28)
ToggleBtn.Image = "rbxassetid://71528188513749"
...
ToggleStroke.Color = Color3.fromRGB(185, 70, 255)
...
JunkTitle.Text = "RAW DATA VIEWER"
...
local JunkCopyBtn = create3DButton(JunkFrame, "คัดลอกข้อมูล", Color3.fromRGB(45, 20, 65), "clipboard")
JunkCopyBtn.Size = UDim2.new(0.45, 0, 0, 28)
JunkCopyBtn.Position = UDim2.new(0.03, 0, 0.87, 0)
JunkCopyBtn.ZIndex = 12

local JunkBackBtn = create3DButton(JunkFrame, "ย้อนกลับ", Color3.fromRGB(55, 18, 55), "back")
...
local function toggleUI(state)
    if state then
        ...
        Shadow3D.Size = UDim2.new(0, 0, 0, 0)
        AuraGlow.Size = UDim2.new(0, 0, 0, 0)

        TweenService:Create(LEDBorder, TweenInfo.new(0.45, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(0.91, 0, 0.80, 0)
        }):Play()
        TweenService:Create(Shadow3D, TweenInfo.new(0.45, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(0.91, 0, 0.80, 0)
        }):Play()
        TweenService:Create(AuraGlow, TweenInfo.new(0.45, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(0.95, 0, 0.84, 0)
        }):Play()

        playUISound(OpenSoundId)
    else
        playUISound(CloseSoundId)
        local tween = TweenService:Create(LEDBorder, TweenInfo.new(0.30, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
            Size = UDim2.new(0, 0, 0, 0)
        })
        TweenService:Create(Shadow3D, TweenInfo.new(0.30, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
            Size = UDim2.new(0, 0, 0, 0)
        }):Play()
        TweenService:Create(AuraGlow, TweenInfo.new(0.30, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
            Size = UDim2.new(0, 0, 0, 0)
        }):Play()
...
local function stopLocalListeningSound()
    ...
    ListenToggleBtn.Text = "ฟังเพลงส่วนตัว 80%"
...
ListenToggleBtn.MouseButton1Click:Connect(function()
    if IsListeningRealTime then
        ...
        StatusLabel.Text = "หยุดฟังเพลงส่วนตัวเรียบร้อยแล้ว"
        return
    end
    ...
    if CurrentSelectedPlayer then
        ...
        if #soundObjects > 0 then
            ...
            ListenToggleBtn.Text = "หยุดฟังเพลง (กำลังเปิด)"
            ListenToggleBtn.BackgroundColor3 = Color3.fromRGB(220, 50, 50)
            StatusLabel.Text = "กำลังเล่นเพลงของ " .. targetPlayer.DisplayName .. " (80% Vol)"
        else
            StatusLabel.Text = "ไม่พบเพลงที่ผู้เล่นคนนี้กำลังเปิดอยู่"
        end
    else
        StatusLabel.Text = "โปรดเลือกชื่อผู้เล่นก่อนกดฟังเพลง!"
    end
end)
...
local function refreshPlayers()
    ...
    for _, p in ipairs(currentPlayers) do
        -- รวม LocalPlayer ด้วย เพื่อให้ตรวจจับเพลงที่ตัวเราเองกำลังเปิดอยู่
        activeMap[p] = true
            ...
            if not btn then
                ...
                btn.ZIndex = 9
                btn.BackgroundColor3 = Color3.fromRGB(18, 13, 28)
            ...
            local playerText = " " .. p.DisplayName .. " (@" .. p.Name .. ")" .. isRunnerTag

            -- แสดงผู้เล่นทุกคนเสมอ
            btn.Text = playerText

            -- ถ้าผู้เล่นมีเพลงที่ระบบสแกนพบ ให้ชื่อเป็นสีเขียว
            if #activeSounds > 0 then
                btn.TextColor3 = Color3.fromRGB(0, 255, 150)
                btn.BackgroundColor3 = Color3.fromRGB(18, 34, 28)
            else
                btn.TextColor3 = Color3.fromRGB(220, 215, 230)
                btn.BackgroundColor3 = Color3.fromRGB(18, 13, 28)
            end
        end
...
GetIDBtn.MouseButton1Click:Connect(function()
    ...
    if CurrentSelectedPlayer then
        ...
        StatusLabel.Text = "โปรดเลือกชื่อผู้เล่นก่อนกดเจาะไอดี!"
    end
end)
...
JunkCopyBtn.MouseButton1Click:Connect(function()
    if lastFullStringForCopy ~= "" then 
        ...
        StatusLabel.Text = "คัดลอกข้อมูลลงคลิปบอร์ดแล้ว"
    end
end)
