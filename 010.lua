...
-- 1. ฟังก์ชันสำหรับตรวจจับการ Hook
local function verifyLoadstring()
    -- ตรวจสอบว่า loadstring ใน getgenv ถูกเปลี่ยนแปลงหรือไม่
    if getgenv and getgenv().loadstring ~= nil and getgenv().loadstring ~= loadstring then
        return false, "SecurityError: Anti-Hook triggered (loadstring environment has been modified)."
    end

    -- ตรวจสอบประเภทฟังก์ชันด้วย iscclosure (ถ้า Executor รองรับ)
    if iscclosure and not iscclosure(loadstring) then
        return false, "SecurityError: Native function tampered (Lua closure detected)."
    end
...
-- 2. รันการตรวจสอบ
local is_safe, err_msg = verifyLoadstring()

if not is_safe then
    -- สั่งตัดการทำงานและส่ง Error กลับไปทันที
    error(err_msg, 2)
end

-- 3. หากปลอดภัย สามารถรันโค้ดได้ตามปกติ
if (getgenv and getgenv().loadstring ~= nil and getgenv().loadstring ~= loadstring) or (iscclosure and not iscclosure(loadstring)) then
    error("SecurityError: Hook attempt detected.", 0)
end
