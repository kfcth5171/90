        ...
        if success and descendants then
            for _, obj in ipairs(descendants) do
                if obj:IsA("Sound") and obj.SoundId ~= "" and obj.IsPlaying then
                    -- เช็คความเป็นเจ้าของ Sound จากโมเดล ตัวละคร หรือโฟลเดอร์ในแมพ
                    local isOwnedByTarget = obj:IsDescendantOf(targetPlayer.Character or game) or 
                                            (backpack and obj:IsDescendantOf(backpack)) or
                                            (vehicle and obj:IsDescendantOf(vehicle)) or
                                            (obj.Parent and string.find(string.lower(obj.Parent.Name), string.lower(targetPlayer.Name)))
                                            
                    if isOwnedByTarget then
...
-- เพิ่มปุ่มฟังเพลงเรียลไทม์บนเครื่องตัวเอง (สลับปุ่มในปุ่มเดียว)
...
-- ระบบควบคุมการฟังเพลงเรียลไทม์บนเครื่องตัวเอง
