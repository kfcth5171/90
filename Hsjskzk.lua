...
-- ใช้ Sound ที่ client กำลังรับ/เล่นจริงเป็นหลัก ไม่เดา ID จากข้อความหรือชื่อ object
local function getHeardMusicSounds(targetPlayer)
    ...
    local function addSound(obj, priority)
        if not obj:IsA("Sound") then return end
        if obj.SoundId == "" or not obj.IsPlaying then return end
        if seen[obj] then return end

        local name = string.lower(obj.Name)
        if string.find(name, "engine")
            or string.find(name, "motor")
            or string.find(name, "horn")
            or string.find(name, "jump")
            or string.find(name, "landing")
            or string.find(name, "running")
        ...
        -- PlaybackLoudness > 0 = client กำลังได้รับเสียงจริง
        local loudness = tonumber(obj.PlaybackLoudness) or 0
        local score = (loudness * 1000) + (priority or 0)
    ...
    -- 1) ให้ความสำคัญกับ Sound ที่อยู่บนตัว/รถของผู้เล่นเป้าหมาย
    ...
    -- 2) fallback: หา Sound ที่ client กำลังได้ยินใน Workspace
    -- เผื่อเกมวาง Sound ไว้นอกรถ/Character แต่ผู้เล่นยังเป็น emitter ของเพลง
    local rootPart = targetPlayer.Character
                ...
                if obj:IsA("Sound") and obj.IsPlaying and obj.SoundId ~= "" then
                    ...
                    if pos then
                        ...
                        if distance <= 180 then
                            local proximity = math.max(0, 180 - distance)
                            addSound(obj, proximity)
                        end
                    end
                end
            end
...
GetIDBtn.MouseButton1Click:Connect(function()
    ...
    if not realSound then
        -- fallback สุดท้ายกลับไปใช้ตัวสแกนเดิม
        local fallbackSounds = checkPlayerAllSounds(targetPlayer)
        realSound = fallbackSounds[1]
    end
