...
local function extractIDsFromPattern(text)
    ...
    -- จับเฉพาะค่าที่อยู่หลังตัวแปรเพลงจริง ห้ามกวาดตัวเลขทุกตัวในข้อความ
    local patterns = {
        "69%%64=([^&%s]+)", "[?&]id=([^&%s]+)", "^id=([^&%s]+)$",
        "audio=([^&%s]+)", "song=([^&%s]+)", "music=([^&%s]+)",
        "%%69%%64=([^&%s]+)", "&%%69%%64=([^&%s]+)",
        "9%s*d%s*=%s*([^&%s]+)", "9d=([^&%s]+)", "9_d=([^&%s]+)", "9%%20d%%20=([^&%s]+)"
    }

    for _, pat in ipairs(patterns) do
        for capture in string.gmatch(text, pat) do
            -- รับเฉพาะค่าที่เป็น ID ตัวเลขล้วน เพื่อไม่ตัดเลขย่อยจากข้อความ/ตัวแปรอื่น
            local num = capture:match("^%s*(%d+)%s*$")
...
local function getPlayerVehicle(player)
    ...
    -- 1) กรณีนั่ง Seat/VehicleSeat ปกติ
    if humanoid and humanoid.SeatPart then
        local model = humanoid.SeatPart:FindFirstAncestorOfClass("Model")
        if model and model ~= character then return model end
    end

    -- 2) สเก็ตบอร์ด/ยานพาหนะที่ Weld ติดกับเท้าหรือตัวละคร
    -- ยึด physical connection จริง ไม่ใช้ระยะใกล้
        ...
        if part:IsA("BasePart") then
            ...
            if ok and connected then
                for _, connectedPart in ipairs(connected) do
                    if connectedPart and not connectedPart:IsDescendantOf(character) then
                        local model = connectedPart:FindFirstAncestorOfClass("Model")
...
-- ==================== ตรวจต้นทางเพลงแบบ STRICT / REALTIME ====================
-- หลักสำคัญ: ไม่ใช้ระยะห่างและไม่เลือกเสียงที่ดังที่สุดเพื่อเดาเจ้าของ
-- Sound ต้องอยู่ใต้ Tool ที่ Character ถืออยู่ หรืออยู่ใน Vehicle ที่เชื่อมกับ Character จริง

local function getEquippedMusicTools(targetPlayer)
...
local function isSoundOwnedByTarget(sound, targetPlayer)
    if not sound or not targetPlayer or not targetPlayer.Character then return false, nil end
    local character = targetPlayer.Character

    -- Boombox / Tool ที่ผู้เล่นถืออยู่จริง
    for _, tool in ipairs(getEquippedMusicTools(targetPlayer)) do
        if sound:IsDescendantOf(tool) then
            return true, "PLAYER_TOOL"
        end
    end

    -- Vehicle ที่นั่งอยู่หรือเชื่อมกับตัว/เท้าจริง
    local vehicle = getPlayerVehicle(targetPlayer)
    if vehicle and sound:IsDescendantOf(vehicle) then
        return true, "PLAYER_LINKED_VEHICLE"
    end

    -- Sound ที่อยู่ใต้ Character โดยตรง (รองรับแมพที่ Remote สร้างเสียงใน Character)
    if sound:IsDescendantOf(character) then
        return true, "PLAYER_CHARACTER"
    end
...
local function getHeardMusicSounds(targetPlayer)
    ...
    -- สแกนเฉพาะต้นทางที่พิสูจน์ความสัมพันธ์กับ Player ได้
    local roots = {character}
    local vehicle = getPlayerVehicle(targetPlayer)
    if vehicle then table.insert(roots, vehicle) end

    for _, root in ipairs(roots) do
        for _, obj in ipairs(root:GetDescendants()) do
            if obj:IsA("Sound") and not seen[obj] then
                seen[obj] = true
                local owned, sourceType = isSoundOwnedByTarget(obj, targetPlayer)
                if owned
                    and obj.SoundId ~= ""
                    and obj.IsPlaying
                    and (tonumber(obj.Volume) or 0) > 0 then

                    local loudness = tonumber(obj.PlaybackLoudness) or 0
                    -- PlaybackLoudness เป็นหลักฐานเสริมว่าเสียงกำลังออกจริง ไม่ใช่ตัวเลือกเจ้าของ
    ...
    -- เรียงเพื่อความเสถียร แต่ไม่ใช้ลำดับเป็นหลักฐานความเป็นเจ้าของ
    table.sort(candidates, function(a, b)
        return a.loudness > b.loudness
    end)

    local result = {}
    for _, item in ipairs(candidates) do table.insert(result, item.sound) end
    return result
end

-- ==================== ยืนยัน ID จาก Sound จริง + Variable แบบ 2 ฝั่ง ====================
local function getDirectSoundId(soundId)
    if type(soundId) ~= "string" or soundId == "" then return nil end
    local text = deepDecode(soundId)
    return text:match("rbxassetid://(%d+)")
        or text:match("[?&]id=(%d+)")
        or text:match("^%s*(%d+)%s*$")
end

local function verifySoundRealtime(soundObj, targetPlayer, samples)
    samples = samples or 3
    if not soundObj or not soundObj.Parent then return false end

    local stableId = soundObj.SoundId
    if stableId == "" then return false end

    for i = 1, samples do
        local owned = isSoundOwnedByTarget(soundObj, targetPlayer)
        if not owned or not soundObj.Parent or not soundObj.IsPlaying then
            return false
        end
        if soundObj.SoundId ~= stableId then
            return false -- เพลงเปลี่ยนระหว่างตรวจ เริ่มรอบใหม่ดีกว่าเดา
        end
        if (tonumber(soundObj.Volume) or 0) <= 0 then return false end
        if (tonumber(soundObj.PlaybackLoudness) or 0) <= 0.05 then return false end
        if i < samples then task.wait(0.08) end
    end
    return true
end

local function getVerifiedPlayerMusic(targetPlayer)
    -- ตรวจซ้ำหลายรอบเพื่อให้เป็น realtime snapshot ที่เสถียร
    local sounds = getHeardMusicSounds(targetPlayer)

    for _, soundObj in ipairs(sounds) do
        if verifySoundRealtime(soundObj, targetPlayer, 3) then
            local rawId = soundObj.SoundId
            local decoded = deepDecode(rawId)
            local realId = getDirectSoundId(rawId)
            local variableIds = extractIDsFromPattern(decoded)

            -- กรณี SoundId เป็น Asset ID ตรง: นี่คือหลักฐานที่แม่นที่สุดของ Sound ที่กำลังเล่น
            if realId and not BlockedIDs[realId] then
                if #variableIds == 0 then
                    return soundObj, realId, "DIRECT_REALTIME_CONFIRMED"
                end

                -- ถ้ามีตัวแปร ให้รับเฉพาะตัวที่ตรงกับ Asset ID ของ Sound เดียวกัน
                for _, variableId in ipairs(variableIds) do
                    if variableId == realId then
                        return soundObj, realId, "VARIABLE_MATCHED_REALTIME_CONFIRMED"
                    end
                end
                -- Variable ไม่ตรงกับเสียงจริง = ปฏิเสธทั้งหมด
            end

            -- ไม่มีการรับ "เลขเดียวที่เจอ" อีกต่อไป เพราะไม่ใช่หลักฐานว่าเป็นเพลงจริง
        end
    end
    return nil, nil, nil
end
    ...
    if CurrentSelectedPlayer then
        ...
        if IsAdmin(targetPlayer) and not IsLocalAdmin() then
            ...
            elseif CurrentViewMode == 2 then
                JunkTitle.Text = "INSTANT LOG VIEWER (ID เพลงจริงยืนยันแล้ว)"
                jStroke.Color = Color3.fromRGB(0, 200, 100)
                JunkCopyBtn.BackgroundColor3 = Color3.fromRGB(0, 200, 100)

                -- ไม่เอาเลขจาก Sound อื่น ไม่เอาเลขสุ่มจากข้อความ
                -- เลือกเสียงที่เกิดและเล่นจริงผ่านตัวผู้เล่นก่อน แล้วตรวจตัวแปรของเสียงนั้นเท่านั้น
                local realSound, verifiedId, verifyType = getVerifiedPlayerMusic(targetPlayer)

                if not realSound or not verifiedId then
                    outputText = "❌ พบเสียง/ตัวแปร แต่ยังไม่มี ID ที่ยืนยันตรงกับเสียงที่เล่นจริง"
                else
                    local verifyText = "DIRECT REAL SOUND"
                    if verifyType == "VARIABLE_MATCHED_REAL_SOUND" then
                        verifyText = "VARIABLE MATCH ✓ REAL SOUND"
                    elseif verifyType == "SINGLE_VARIABLE_FROM_REAL_SOUND" then
                        verifyText = "SINGLE VARIABLE FROM REAL SOUND"
                    end

                    outputText = string.format(
                        "✅ ยืนยันเพลงที่ผู้เล่นกำลังเปิดจริงแล้ว\n\n" ..
                        "🎵 Source: %s\n" ..
                        "▶ Playing: %s | Loudness: %.1f\n" ..
                        "🔍 Verify: %s\n\n" ..
                        "🆔 ID เพลงจริง: %s",
                        realSound:GetFullName(),
                        tostring(realSound.IsPlaying),
                        tonumber(realSound.PlaybackLoudness) or 0,
                        verifyText,
                        verifiedId
                    )
                end
...
GetIDBtn.MouseButton1Click:Connect(function()
    ...
    if IsAdmin(CurrentSelectedPlayer) and not IsLocalAdmin() then
        StatusLabel.Text = "❌ ไม่สามารถดึงข้อมูลของผู้เล่นนี้ได้ Protection Admin"
        return
    end
    ...
    StatusLabel.Text = "🔍 REALTIME: ตรวจ Sound จริง + Match ตัวแปร..."

    ----------------------------------------------------------------
    -- FIX เฉพาะปุ่ม "เจาะเพลง"
    --
    -- กติกาใหม่แบบ STRICT:
    -- 1) ต้องเป็น Sound object ที่มีอยู่จริงใน client และกำลังเล่น
    -- 2) Sound ต้องสัมพันธ์กับ Player ที่เลือกจริง
    -- 3) อ่าน ID จริงจาก Sound.SoundId เท่านั้น
    -- 4) เอา ID จริงไปเทียบกับค่าตัวแปร/Attribute/ValueBase
    -- 5) ถ้ามีตัวแปรแต่ไม่ตรงกับ SoundId จริง = ปฏิเสธ
    -- 6) ID ที่อยู่แต่ในตัวแปร แต่ไม่มี Sound กำลังเล่น = ปฏิเสธ
    -- 7) ยืนยันซ้ำ 3 samples เพื่อกัน ID ที่เปลี่ยน/สร้างหลอกระหว่างสแกน
    --
    -- จุดสำคัญ:
    -- Fake ID จะถูกคัดออก เพราะต้องมี Sound จริงที่กำลังเล่นและ
    -- Sound.SoundId ต้องตรงกับ ID ในตัวแปร ณ เวลาเดียวกัน
    ----------------------------------------------------------------
    ...
    local function normalizeMusicId(value)
        ...
        return text:match("rbxassetid://(%d+)")
            or text:match("rbxasset://(%d+)")
            or text:match("[?&]id=(%d+)")
            or text:match("[?&]assetId=(%d+)")
            or text:match("^%s*(%d+)%s*$")
    end

    local function addVariableId(result, seen, value)
        local id = normalizeMusicId(value)

        if id and not BlockedIDs[id] and not seen[id] then
            seen[id] = true
            table.insert(result, id)
        end

        -- รองรับค่าที่เป็นข้อความซึ่งใช้รูปแบบตัวแปรเพลงเดิม
        if type(value) == "string" then
            local decoded = deepDecode(value)
            for _, parsedId in ipairs(extractIDsFromPattern(decoded)) do
                if not BlockedIDs[parsedId] and not seen[parsedId] then
                    seen[parsedId] = true
                    table.insert(result, parsedId)
                end
            end
        end
    end

    local function collectVariableIds(instance, result, seen)
        if not instance then return end

        -- Attributes ของ Object
        ...
        if okAttrs and attrs then
            for _, value in pairs(attrs) do
                addVariableId(result, seen, value)
            end
        end

        -- ValueBase ที่เป็นลูกโดยตรง
    ...
    -- อ่านตัวแปรรอบ Sound ก่อน แล้วขยายขึ้นไปตาม ancestor
    -- จำกัดขอบเขตเพื่อไม่ให้กวาดเลขทั้งเกมแบบมั่ว
    local function getNearbyVariableIds(soundObj)
        local ids, seen = {}, {}

        collectVariableIds(soundObj, ids, seen)

        local node = soundObj.Parent
        local depth = 0

        while node and depth < 4 do
            collectVariableIds(node, ids, seen)
            node = node.Parent
            depth += 1
        end

        -- รองรับ Folder/ValueBase ที่อยู่ใต้ Sound
        local okDesc, descendants = pcall(function()
            return soundObj:GetDescendants()
        end)
        ...
        if okDesc and descendants then
            ...
            for _, child in ipairs(descendants) do
                if child:IsA("ValueBase") then
                    collectVariableIds(child, ids, seen)
                    count += 1
    ...
    -- สร้างดัชนี "ตัวแปรที่อยู่ใน map" เพิ่มเติม
    -- จุดประสงค์คือรองรับแมพที่เก็บ 4 ตัวแปรเพลงไว้ใน Folder/Model
    -- แยกจาก Sound โดยตรง
    local function collectMapVariableIds()
        local ids, seen = {}, {}
        ...
        -- Map จริง + SoundService ที่ replicate มาถึง client
        scanRoot(workspace)
        scanRoot(game:GetService("SoundService"))

        -- Source ของ player เพิ่มเติม
        if targetPlayer.Character then
            scanRoot(targetPlayer.Character)
        end
    ...
    local function containsId(list, wanted)
    ...
    local function isMovementSound(soundObj)
        local n = string.lower(soundObj.Name or "")
        local blockedNames = {
            "engine", "motor", "horn",
            "jump", "jumping", "landing",
            "running", "freefall", "freefalling",
            "swim", "swimming", "climb", "climbing",
            "gettingup", "died", "splash"
        }

        for _, word in ipairs(blockedNames) do
    ...
    -- ตรวจว่า Sound เป็นของ Player ที่เลือกจริง
    local function isTargetSound(soundObj)
        if not soundObj or not targetPlayer or not targetPlayer.Character then
            return false
        end
        ...
        for _, tool in ipairs(getEquippedMusicTools(targetPlayer)) do
            if soundObj:IsDescendantOf(tool) then
                return true
            end
        end

        local vehicle = getPlayerVehicle(targetPlayer)
        if vehicle and soundObj:IsDescendantOf(vehicle) then
            return true
        end

        if soundObj:IsDescendantOf(character) then
            return true
        end
    ...
    -- ยืนยัน Sound เดิมซ้ำหลายครั้ง:
    -- object เดิม + ID เดิม + playing อยู่ + variable ยัง match
    local function verifyCandidate(soundObj, realId, mapVariableIds)
        if not soundObj or not soundObj:IsDescendantOf(game) then
            return false
        end

        if not soundObj.IsPlaying or soundObj.SoundId == "" then
            return false
        end

        if not isTargetSound(soundObj) then
            return false
        end

        if normalizeMusicId(soundObj.SoundId) ~= realId then
            return false
        end

        local localVariableIds = getNearbyVariableIds(soundObj)
        local matchedLocal = containsId(localVariableIds, realId)
        local matchedMap = containsId(mapVariableIds, realId)

        -- ต้องมีหลักฐานจากตัวแปรจริง
        if not matchedLocal and not matchedMap then
            return false
        end

        for i = 1, 3 do
            if not soundObj:IsDescendantOf(game)
                or not soundObj.IsPlaying
                or soundObj.SoundId == "" then
                return false
            end
            ...
            -- refresh variable ทุก sample เพื่อกันตัวแปรถูกเปลี่ยนเป็น ID หลอก
            local currentNearby = getNearbyVariableIds(soundObj)
            local currentMap = collectMapVariableIds()

            if not containsId(currentNearby, realId)
                and not containsId(currentMap, realId) then
                return false
            end
    ...
    local mapVariableIds = collectMapVariableIds()
    local candidates = {}
    local seenSounds = {}
    ...
    local function collectSounds(root)
        ...
        for _, obj in ipairs(descendants) do
            if obj:IsA("Sound")
                and not seenSounds[obj]
                and obj:IsDescendantOf(game)
                and obj.IsPlaying
                and obj.SoundId ~= ""
                and (tonumber(obj.Volume) or 0) > 0
                and isTargetSound(obj)
                and not isMovementSound(obj) then

                local realId = normalizeMusicId(obj.SoundId)

                if realId and not BlockedIDs[realId] then
                    seenSounds[obj] = true

                    local nearbyIds = getNearbyVariableIds(obj)
                    local variableMatched =
                        containsId(nearbyIds, realId)
                        or containsId(mapVariableIds, realId)

                    -- สำคัญ: ไม่เก็บ candidate ถ้า ID ไม่ตรงตัวแปร
                    -- จึงไม่เอา ID หลอกจากตัวแปรมาเป็นผลลัพธ์
    ...
    -- ใช้ต้นทางเดิมของโค้ดเป็นหลัก ไม่เปลี่ยนโครงสร้างระบบปุ่ม
    collectSounds(workspace)
    collectSounds(game:GetService("SoundService"))

    if targetPlayer.Character then
        collectSounds(targetPlayer.Character)
    end

    local vehicle = getPlayerVehicle(targetPlayer)
    if vehicle then
        collectSounds(vehicle)
    end
    ...
    local playerGui = targetPlayer:FindFirstChildOfClass("PlayerGui")
    if playerGui then
        collectSounds(playerGui)
    end

    -- เลือก candidate ที่สัมพันธ์กับ Player มากที่สุด
    local best = nil
    local bestScore = -math.huge

    for _, item in ipairs(candidates) do
        local soundObj = item.sound
        local score = 0

        if targetPlayer.Character and soundObj:IsDescendantOf(targetPlayer.Character) then
            score += 500000000
        end
        ...
        if playerGui and soundObj:IsDescendantOf(playerGui) then
            score += 200000000
        end

        if (tonumber(soundObj.PlaybackLoudness) or 0) > 0.05 then
            score += 10000000
        end

        if (tonumber(soundObj.TimePosition) or 0) > 0 then
            score += 100000
        end

        score += math.min(tonumber(soundObj.PlaybackLoudness) or 0, 1000) * 100
    ...
    if not best or not best.sound then
        StatusLabel.Text =
            "❌ ไม่พบเพลงที่ตรงทั้ง Sound จริง + ตัวแปรใน Map"
        return
    end

    -- ก่อนคัดลอกจริง ให้สร้าง snapshot ตัวแปรใหม่
    -- เพื่อไม่ให้ ID หลุดจากช่วงเวลาที่กำลังเล่น
    mapVariableIds = collectMapVariableIds()

    local finalSound = best.sound
    local finalId = normalizeMusicId(finalSound.SoundId)

    if not finalId or finalId == "" then
        StatusLabel.Text = "❌ Sound ที่พบไม่มี Asset ID จริง"
        return
    end

    if BlockedIDs[finalId] then
        StatusLabel.Text = "❌ ID นี้อยู่ใน BlockedIDs"
        return
    end

    if not verifyCandidate(finalSound, finalId, mapVariableIds) then
        StatusLabel.Text =
            "⚠️ ID เปลี่ยน/ตัวแปรไม่ตรง/Sound ไม่ได้เล่นจริง — ยกเลิกการดึง"
        return
    end

    -- ตรวจครั้งสุดท้าย: ID ที่จะ copy ต้องมาจาก Sound.SoundId
    -- ไม่ใช่จากตัวแปรโดยตรง
    local lastRealId = normalizeMusicId(finalSound.SoundId)

    if not lastRealId or lastRealId ~= finalId then
        StatusLabel.Text = "⚠️ Sound เปลี่ยนก่อนคัดลอก — ไม่ดึง ID"
        return
    end

    copyToClipboard(lastRealId)

    StatusLabel.Text =
        "📋 เจาะเพลงสำเร็จ [REAL SOUND + VARIABLE MATCH] ID: " .. lastRealId
end)
