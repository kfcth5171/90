...
local mStroke = stroke(MainFrame, PURPLE, 2, 0.05)
...
local function updateResponsiveLayout()
    ...
    if compact then
        ...
        local leftH = math.floor(contentH * (shortScreen and 0.43 or 0.46))
        local rightH = math.max(contentH - leftH - gap, shortScreen and 132 or 150)
        ...
        LeftPanel.Size = UDim2.new(1, -16, 0, leftH)
        LeftPanel.Position = UDim2.new(0, 8, 0, contentTop)
        PlayerHeader.TextSize = shortScreen and 10 or 12
        SearchBox.Size = UDim2.new(1, -18, 0, shortScreen and 25 or 30)
        SearchBox.Position = UDim2.new(0, 9, 0, shortScreen and 30 or 36)
        SearchBox.TextSize = shortScreen and 8 or 10
        ListScroll.Size = UDim2.new(1, -18, 1, -(shortScreen and 62 or 72))
        ListScroll.Position = UDim2.new(0, 9, 0, shortScreen and 59 or 72)

        RightPanel.Size = UDim2.new(1, -16, 0, rightH)
        RightPanel.Position = UDim2.new(0, 8, 0, contentTop + leftH + gap)
        MusicHeader.TextSize = shortScreen and 10 or 12

        local buttonH = shortScreen and 18 or 28
        local buttonGap = shortScreen and 2 or 4
        ButtonsContainer.Size = UDim2.new(1, -18, 0, (buttonH * 5) + (buttonGap * 4))
        ButtonsContainer.Position = UDim2.new(0, 9, 0, shortScreen and 28 or 34)
        ...
        for _, btn in ipairs({GetIDBtn, ListenBtn, GetJunkBtn, ViewRawJunkBtn, ViewInstantBtn}) do
            ...
            btn.TextSize = shortScreen and 8 or 10
        end

        ListenPanel.Size = UDim2.new(1, -18, 0, shortScreen and 20 or 32)
        ListenPanel.Position = UDim2.new(0, 9, 1, -(shortScreen and 25 or 40))
        ListenPanel.Visible = not shortScreen
        ListenTitle.TextSize = shortScreen and 7 or 9
...
-- ==================== ปุ่มเปิด UI + ไฟเส้นยาว 2 เส้นต่อฝั่ง ====================
local ToggleHolder = Instance.new("Frame", ScreenGui)
ToggleHolder.Size = UDim2.new(0, 150, 0, 54)
ToggleHolder.Position = UDim2.new(0.02, 0, 0.50, -27)
ToggleHolder.BackgroundTransparency = 1
ToggleHolder.ZIndex = 10

local ToggleBtn = Instance.new("ImageButton", ToggleHolder)
ToggleBtn.Size = UDim2.new(0, 48, 0, 48)
ToggleBtn.Position = UDim2.new(0.5, -24, 0.5, -24)
...
local ToggleLines = {}
local function makeToggleLine(position, width)
    local line = Instance.new("Frame", ToggleHolder)
    ...
    table.insert(ToggleLines, gradient)
end

-- 2 เส้นด้านซ้าย + 2 เส้นด้านขวา
makeToggleLine(UDim2.new(0, 4, 0.5, -9), 42)
makeToggleLine(UDim2.new(0, 4, 0.5, 7), 42)
makeToggleLine(UDim2.new(1, -46, 0.5, -9), 42)
makeToggleLine(UDim2.new(1, -46, 0.5, 7), 42)
...
task.spawn(function()
    ...
    while ScreenGui.Parent do
        offset += 0.035
        if offset > 1 then offset = -1 end
        for i, gradient in ipairs(ToggleLines) do
            local dir = (i % 2 == 0) and -1 or 1
            gradient.Offset = Vector2.new(offset * dir, 0)
        end
        task.wait(0.035)
    end
end)
