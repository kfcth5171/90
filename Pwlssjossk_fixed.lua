...
local SoundCache = {} -- [Player] = { [Sound] = true }
local SoundConnections = {} -- [Player] = {RBXScriptConnection...}
...
local function isValidSound(obj)
    if not obj or not obj:IsA("Sound") then return false end
    if obj.SoundId == "" or not obj.IsPlaying then return false end

    local name = string.lower(obj.Name)
    for blockedName in pairs(NameBlacklist) do
        if string.find(name, blockedName, 1, true) then
            return false
        end
    end
    return not BlockedIDs[string.match(obj.SoundId, "%d+")] 
end

local function addSoundToCache(player, obj)
    if not player or not obj or not isValidSound(obj) then return end
    SoundCache[player] = SoundCache[player] or {}
    SoundCache[player][obj] = true
end

local function removeSoundFromCache(player, obj)
    local cache = SoundCache[player]
    if cache then
        cache[obj] = nil
    end
end

local function scanFolderOnce(player, folder)
    ...
    for _, obj in ipairs(descendants) do
        addSoundToCache(player, obj)
    end
end

local function disconnectPlayerSoundCache(player)
    local conns = SoundConnections[player]
    if conns then
        for _, conn in ipairs(conns) do
            pcall(function() conn:Disconnect() end)
        end
    end
    SoundConnections[player] = nil
    SoundCache[player] = nil
end

local function setupPlayerSoundCache(player)
    if not player then return end
    disconnectPlayerSoundCache(player)

    SoundCache[player] = {}
    SoundConnections[player] = {}

    local function watchContainer(container)
        if not container then return end

        -- Initial scan happens once per container, not on every button refresh.
        scanFolderOnce(player, container)

        table.insert(SoundConnections[player], container.DescendantAdded:Connect(function(obj)
            addSoundToCache(player, obj)

            if obj:IsA("Sound") then
                obj:GetPropertyChangedSignal("SoundId"):Connect(function()
                    if isValidSound(obj) then
                        addSoundToCache(player, obj)
                    else
                        removeSoundFromCache(player, obj)
                    end
                end)
        ...
        table.insert(SoundConnections[player], container.DescendantRemoving:Connect(function(obj)
            removeSoundFromCache(player, obj)
        end))

        -- Track existing Sound state changes.
        local ok, descendants = pcall(function() return container:GetDescendants() end)
        if ok and descendants then
            for _, obj in ipairs(descendants) do
                if obj:IsA("Sound") then
                    table.insert(SoundConnections[player], obj:GetPropertyChangedSignal("SoundId"):Connect(function()
                        if isValidSound(obj) then
                            addSoundToCache(player, obj)
                        else
                            removeSoundFromCache(player, obj)
                    ...
                    table.insert(SoundConnections[player], obj:GetPropertyChangedSignal("IsPlaying"):Connect(function()
                        if isValidSound(obj) then
                            addSoundToCache(player, obj)
                        else
                            removeSoundFromCache(player, obj)
    ...
    watchContainer(player:FindFirstChild("Backpack"))

    local vehicle = getPlayerVehicle(player)
    watchContainer(vehicle)

    table.insert(SoundConnections[player], player.CharacterAdded:Connect(function(char)
        task.defer(function()
            if not SoundCache[player] then return end
            watchContainer(char)
        end)
    end))

    table.insert(SoundConnections[player], player.ChildAdded:Connect(function(child)
        if child.Name == "Backpack" then
            task.defer(function()
                if SoundCache[player] then watchContainer(child) end
            end)
        end
    end))
end
...
local function checkPlayerAllSounds(targetPlayer)
    ...
    -- Fast path: return cached references immediately.
    local cache = SoundCache[targetPlayer]
    if not cache then
        setupPlayerSoundCache(targetPlayer)
        cache = SoundCache[targetPlayer] or {}
    end
    ...
    for obj in pairs(cache) do
        if obj and obj.Parent and isValidSound(obj) then
...
if PlayerGui:FindFirstChild("Honkuki-191") then PlayerGui:FindFirstChild("Honkuki-191"):Destroy() end
...
Players.PlayerAdded:Connect(function(p)
    setupPlayerSoundCache(p)
    refreshPlayers()
end)
Players.PlayerRemoving:Connect(function(p)
    disconnectPlayerSoundCache(p)
    if CurrentSelectedPlayer == p then
