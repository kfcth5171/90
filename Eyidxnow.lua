...
-- ปรับปรุง deepDecode ให้เร็วขึ้น
local function deepDecode(str)
    if type(str) ~= "string" then return str end
    -- ถ้าไม่มีเครื่องหมายที่ต้อง Decode ก็คืนค่าทันที
    if not string.find(str, "%%") and not string.find(str, "0x") and not string.find(str, "\\x") then
        return str
    end
    local prev
    local count = 0
...
-- ปรับปรุง extractRealIDs ให้ทำงานเร็วขึ้น
local function extractRealIDs(soundObjects)
    ...
    for _, soundObj in ipairs(soundObjects) do
        ...
        if realId and not BlockedIDs[realId] and not seen[realId] then
            ...
            -- ถ้าไม่มีรูปแบบมาตรฐาน และ decoded ไม่เหมือน rawId (แสดงว่ามีการเข้ารหัส) ให้ใช้ extractIDsFromPattern
            if decoded ~= rawId then
                ...
                -- ถ้าไม่มีรูปแบบมาตรฐานและไม่มีการเข้ารหัส ให้ลองจับตัวเลขจากข้อความโดยตรง
...
GetIDBtn.MouseButton1Click:Connect(function()
    ...
    StatusLabel.Text = "🔍 กำลังเจาะ ID ทั้งหมด ... โปรดรอสักครู่"
    
    task.spawn(function()
        local targetPlayer = Players:FindFirstChild(CurrentSelectedPlayer.Name)
        if not targetPlayer then
            StatusLabel.Text = "❌ ผู้เล่นนี้ไม่อยู่ในเกมแล้ว"
            GetIDBtn.Enabled = true
            return
        end
