...
GetIDBtn.MouseButton1Click:Connect(function()
    ...
    StatusLabel.Text = "🔍 กำลังจับตัวแปรเพลง แล้วเทียบกับ Sound ที่เล่นจริง..."

    ----------------------------------------------------------------
    -- ขั้นตอน 1:
    -- ใช้ "ตัวแปร/ผลการจับเสียงที่มีอยู่แล้ว" เป็นตัวตั้ง
    --
    -- ตัวแปรที่มีอยู่ในสคริปต์:
    --   LastHeardMusicSound
    --   LastHeardMusicSoundId
    --   heardSounds
    --
    -- ไม่สร้าง ID จาก junk และไม่เดาเลขจากข้อความ
    ----------------------------------------------------------------

    local candidateIds = {}
    local candidateMap = {}

    local function addCandidateId(value)
        if type(value) ~= "string" or value == "" then
            return
        end

        -- รับเฉพาะ SoundId รูปแบบตรง ๆ
    ...
    -- ตัวแปรเสียงล่าสุดที่ระบบฟังเคยจับได้
    if LastHeardMusicSound
        and LastHeardMusicSound:IsA("Sound") then
        addCandidateId(LastHeardMusicSound.SoundId)
    end

    -- ตัวแปร ID ล่าสุด
    addCandidateId(LastHeardMusicSoundId)

    -- ตัวแปรชุดเสียงที่ระบบหลักจับได้
    local heardSounds = getHeardMusicSounds(targetPlayer)

    for _, soundObj in ipairs(heardSounds) do
        if soundObj
            and soundObj:IsA("Sound") then
            addCandidateId(soundObj.SoundId)
        end
    end

    if #candidateIds == 0 then
        StatusLabel.Text = "❌ ตัวแปรเพลงยังไม่มี ID ที่ใช้งานได้"
        return
    end

    ----------------------------------------------------------------
    -- ขั้นตอน 2:
    -- เอา ID จากตัวแปรด้านบนไป "ค้นหา Sound ตัวจริงในแมพ"
    --
    -- สำคัญ:
    -- ID จะผ่านก็ต่อเมื่อมี Sound instance ที่:
    --   1. SoundId ตรงกับตัวแปร
    --   2. IsPlaying = true
    --   3. Volume > 0
    --   4. PlaybackLoudness > 0.1
    --   5. ยังอยู่ในเกม
    --
    -- ดังนั้น ID ที่อยู่ในตัวแปรแต่ไม่ได้เล่นอยู่ในแมพจะถูกทิ้ง
    ----------------------------------------------------------------

    local origin, preferredRoot = getTargetMusicOrigin(targetPlayer)

    if not origin then
        StatusLabel.Text = "❌ ไม่พบตำแหน่งผู้เล่นสำหรับตรวจเสียง"
        return
    end
    ...
    local function getIdFromSound(soundObj)
        if not soundObj or not soundObj:IsA("Sound") then
            return nil
        end

        local sid = soundObj.SoundId

        if type(sid) ~= "string" or sid == "" then
            return nil
        end

        local id = sid:match("^rbxassetid://(%d+)$")

        if not id then
            id = sid:match("^asset://(%d+)$")
        end

        return id
    end

    local function inspectLiveSound(soundObj)
        if not soundObj
            or not soundObj:IsA("Sound")
            or not soundObj:IsDescendantOf(game) then
            return
        end
        ...
        local volume = tonumber(soundObj.Volume) or 0
        local loudness = tonumber(soundObj.PlaybackLoudness) or 0

        if volume <= 0 or loudness <= 0.1 then
            return
        end

        local liveId = getIdFromSound(soundObj)

        if not liveId or not candidateMap[liveId] then
            return
        end

        -- กันชื่อเสียงระบบรถ/ตัวละคร
        local soundName = string.lower(soundObj.Name)
        ...
        for _, blockedName in ipairs(blacklist) do
            if string.find(soundName, blockedName, 1, true) then
                return
            end
        end
        ...
        -- จำกัดบริเวณให้เป็นเสียงที่เกี่ยวข้องกับผู้เล่นเป้าหมาย
        local maxDistance = tonumber(soundObj.RollOffMaxDistance) or 180

        if distance > math.max(maxDistance, 180) then
            return
        end

        ----------------------------------------------------------------
        -- คะแนน:
        -- Sound ที่อยู่ใน Character/รถของผู้เล่น = สูงสุด
        -- ใกล้ผู้เล่น = สูง
        -- มีเสียงออกจริงมาก = สูง
        ----------------------------------------------------------------
        ...
        score += math.max(0, 100000 - (distance * 100))
        score += loudness * 1000
    ...
    ----------------------------------------------------------------
    -- ตรวจ Sound ที่ถูกสร้างในแมพ
    -- แต่ต้องตรงกับ ID ที่ "ตัวแปรของเราเคยจับได้" เท่านั้น
    ----------------------------------------------------------------
    ...
    ----------------------------------------------------------------
    -- ขั้นตอน 3:
    -- ได้ Sound จริงเพียง 1 ตัว
    ----------------------------------------------------------------

    if not bestSound then
        StatusLabel.Text =
            "❌ ID จากตัวแปรมีอยู่ แต่ไม่พบ Sound ID เดียวกันที่กำลังเล่นจริงในแมพ"
        return
    end

    ----------------------------------------------------------------
    -- ตรวจรอบสุดท้ายก่อน Copy
    ----------------------------------------------------------------

    if not bestSound:IsDescendantOf(game)
        or not bestSound.IsPlaying then
        StatusLabel.Text = "❌ เพลงเปลี่ยน/หยุดก่อนเจาะเสร็จ"
        return
    end

    local finalId = getIdFromSound(bestSound)

    if not finalId
        or not candidateMap[finalId]
        or BlockedIDs[finalId] then
        StatusLabel.Text = "❌ Sound ที่พบไม่ตรงกับตัวแปรเพลง"
        return
    end

    local finalLoudness =
        tonumber(bestSound.PlaybackLoudness) or 0

    local finalVolume =
        tonumber(bestSound.Volume) or 0

    if finalVolume <= 0 or finalLoudness <= 0.1 then
        StatusLabel.Text = "❌ ตรวจซ้ำแล้วเสียงไม่ได้กำลังเล่นจริง"
        return
    end

    ----------------------------------------------------------------
    -- สำเร็จ:
    -- Copy แค่ ID เดียวจาก Sound ที่:
    -- ตัวแปรจับได้ + มี Sound จริง + กำลังเล่นจริง
    ----------------------------------------------------------------
