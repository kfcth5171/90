-- =====================================================================
-- HONKUKI DEEP VALIDATOR SCANNER + REAL-TIME LOCAL AUDIO SPY SYSTEM
-- =====================================================================
...
-- ==================== การประกาศตัวแปรระดับสโคปบนสุด (Top-Level Scope) ====================
local TAG_NAME = "Honkuki_Active_Runner_Tag"
local CurrentSelectedPlayer = nil
local StatusLabel = nil
local AssetCache = {}

-- Remote สำหรับระบบลบของ
local ClearToolsRemote = ReplicatedStorage
    :WaitForChild("RE", 5)
    and ReplicatedStorage.RE:WaitForChild("1Clear1rTool1s", 5)

-- ==================== ระบบฟังเพลง Local Real-time (Client-Side Only) ====================
local LocalAudioPlayer = Instance.new("Sound")
LocalAudioPlayer.Name = "Honkuki_Local_Audio_Mirror"
LocalAudioPlayer.Volume = 1.5
LocalAudioPlayer.Looped = true
LocalAudioPlayer.Parent = (gethui and gethui()) or CoreGui or PlayerGui

local IsListening = false
local CurrentListeningTarget = nil
local ListenBtn = nil
...
local BlockedIDs = {
    ...
    ["0052315987524169"] = true, ["00123568751245557"] = true,
    ["00965488877651295"] = true, ["008106708446416535"] = true,
    ["0083260119448675"] = true, ["00129462618639650"] = true, ["0083932827347443"] = true,
    ["00100800577164015"] = true, ["0090309298517536"] = true, ["00117628672363749"] = true,
    ["0091848271981900"] = true, ["00134476916426685"] = true, ["00196872951970254"] = true,
    ["00129643829992035"] = true, ["00107058019826867"] = true, ["00121320815776761"] = true,
    ["0097681411562121"] = true, ["00104523837494454"] = true, ["00108763959187625"] = true,
    ["00103341533670628"] = true, ["0073567657933546"] = true, ["00117484747381529"] = true,
    ["00102583982042068"] = true, ["0089689020278596"] = true, ["00126329595231978"] = true,
    ["0090338113256962"] = true, ["0087260439948695"] = true, ["00103462658039651"] = true,
    ["0093931829347443"] = true, ["00106800817264017"] = true, ["0090307298519537"] = true,
    ["00117628371363749"] = true, ["0083848201981900"] = true, ["00134076916421685"] = true,
    ["00116872955970254"] = true, ["00129043827992035"] = true, ["00137058099826867"] = true,
    ["00121320825772761"] = true, ["0083681471562121"] = true, ["00134523838494464"] = true,
    ...
    ["5555555"] = true,  
    ["0010781057726401593932829347443"] = true, ["0010681557726401593932829347441"] = true,
    ["0010699057726401593932829347443"] = true, ["555555"] = true,
}
...
MainFrame.Size = UDim2.new(0, 520, 0, 265)
...
ListScroll.Size = UDim2.new(0.45, 0, 0, 150)
...
ButtonsContainer.Size = UDim2.new(0.47, 0, 0, 150)
...
-- ปุ่มฟังเพลงReal-time บนเครื่องเรา
ListenBtn = Instance.new("TextButton", ButtonsContainer)
ListenBtn.Size = UDim2.new(1, 0, 0, 26)
ListenBtn.BackgroundColor3 = Color3.fromRGB(0, 180, 255)
ListenBtn.Text = "🎧 ฟังเพลงผู้เล่น"
ListenBtn.Font = Enum.Font.GothamBold
ListenBtn.TextSize = 11
ListenBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
Instance.new("UICorner", ListenBtn).CornerRadius = UDim.new(0, 6)

local DeleteToolsBtn = Instance.new("TextButton", ButtonsContainer)
DeleteToolsBtn.Size = UDim2.new(1, 0, 0, 26)
DeleteToolsBtn.BackgroundColor3 = Color3.fromRGB(220, 60, 60)
DeleteToolsBtn.Text = "🗑️ ลบของ"
DeleteToolsBtn.Font = Enum.Font.GothamBold
DeleteToolsBtn.TextSize = 11
DeleteToolsBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
Instance.new("UICorner", DeleteToolsBtn).CornerRadius = UDim.new(0, 6)
...
RefreshBtn.Position = UDim2.new(0.73, 0, 0.85, 0)
...
local function stopLocalListening()
    IsListening = false
    CurrentListeningTarget = nil
    LocalAudioPlayer:Stop()
    LocalAudioPlayer.SoundId = ""
    ListenBtn.Text = "🎧 ฟังเพลงผู้เล่น"
    ListenBtn.BackgroundColor3 = Color3.fromRGB(0, 180, 255)
end
        ...
        if p ~= LocalPlayer then
            ...
            if not btn then
                ...
                btn.MouseButton1Click:Connect(function()
                    ...
                    if IsListening and CurrentListeningTarget ~= p then
                        stopLocalListening()
                    end
...
DeleteToolsBtn.MouseButton1Click:Connect(function()
    local ok, err = pcall(function()
        ClearToolsRemote:FireServer("ClearTools")
    end)

    if ok then
        StatusLabel.Text = "🗑️ ส่งคำสั่งลบของแล้ว"
    else
        StatusLabel.Text = "❌ ส่งคำสั่งลบของไม่สำเร็จ"
        warn("ClearToolsRemote error:", err)
    end
end)

-- ลอจิกฟังเพลง Local Real-time (ในปุ่มเดียว)
ListenBtn.MouseButton1Click:Connect(function()
    if IsListening then
        stopLocalListening()
        StatusLabel.Text = "⏹️ หยุดฟังเพลงเรียบร้อยแล้ว"
        return
    end

    if not CurrentSelectedPlayer then
        StatusLabel.Text = "⚠️ โปรดเลือกผู้เล่นที่ต้องการฟังเพลงก่อน!"
        return
    end

    if IsAdmin(CurrentSelectedPlayer) and not IsLocalAdmin() then
        StatusLabel.Text = "❌ ไม่สามารถแอบฟังเพลงของ Admin ได้"
        return
    end
    ...
    if playSoundId then
        LocalAudioPlayer.SoundId = "rbxassetid://" .. tostring(playSoundId)
        LocalAudioPlayer.Volume = 1.5
        LocalAudioPlayer.Looped = true
        LocalAudioPlayer:Play()

        IsListening = true
        CurrentListeningTarget = CurrentSelectedPlayer
        ListenBtn.Text = "⏹️ หยุดฟังเพลง"
        ListenBtn.BackgroundColor3 = Color3.fromRGB(220, 50, 50)
        StatusLabel.Text = "🎧 กำลังฟังเพลงจาก: " .. targetPlayer.DisplayName
    else
        StatusLabel.Text = "❌ ไม่พบ ID เพลงที่สามารถเปิดฟังได้"
    end
end)
...
Players.PlayerRemoving:Connect(function(p)
    if CurrentSelectedPlayer == p then
        ...
        if IsListening then stopLocalListening() end
...
task.spawn(function()
    while true do
        ...
        -- อัปเดตการฟังเพลง Real-time ติดตามผู้เล่น
        if IsListening and CurrentListeningTarget then
            local targetPlayer = Players:FindFirstChild(CurrentListeningTarget.Name)
            if targetPlayer then
                local sounds = checkPlayerAllSounds(targetPlayer)
                if #sounds > 0 then
                    local targetSound = sounds[1]
                    if LocalAudioPlayer.SoundId ~= targetSound.SoundId then
                        LocalAudioPlayer.SoundId = targetSound.SoundId
                        LocalAudioPlayer:Play()
                    end
                else
                    stopLocalListening()
                    StatusLabel.Text = "⚠️ ผู้เล่นหยุดเปิดเพลงแล้ว"
                end
            else
                stopLocalListening()
            end
