...
local function getExactPlayingSoundId(soundObj)
    if not soundObj or not soundObj:IsA("Sound") then return nil end
    if not soundObj.IsPlaying then return nil end

    local rawId = soundObj.SoundId or ""
    if rawId == "" then return nil end

    -- ใช้ SoundId ของ Sound ที่กำลังเล่นจริงโดยตรง
    -- ไม่ดึงเลขสุ่มจากข้อความ/URL/ค่าที่ encode อยู่
    local id = string.match(rawId, "^rbxassetid://(%d+)$")
    if not id then
        id = string.match(rawId, "[?&]id=(%d+)")
    end
    if not id then
        id = string.match(rawId, "^(%d+)$")
    end

    if not id or id == "" or BlockedIDs[id] then
        return nil
    end
...
GetIDBtn.MouseButton1Click:Connect(function()
    if CurrentSelectedPlayer then
        ...
        StatusLabel.Text = "🔍 กำลังตรวจเสียงที่ผู้เล่นกำลังเล่นจริง..."

        local targetPlayer = Players:FindFirstChild(CurrentSelectedPlayer.Name)
        local exactId = getExactPlayingMusicId(targetPlayer)

        if exactId then
            copyToClipboard(exactId)
            StatusLabel.Text = "📋 คัดลอก ID เพลงจริง: " .. exactId
        else
            StatusLabel.Text = "❌ ไม่พบ Sound ที่กำลังเล่นจริงของผู้เล่นนี้"
        end
    else
...
GetJunkBtn.MouseButton1Click:Connect(function()
    if CurrentSelectedPlayer then
        ...
        for _, soundObj in ipairs(soundObjects) do
            firstCleanId = getExactPlayingSoundId(soundObj)
