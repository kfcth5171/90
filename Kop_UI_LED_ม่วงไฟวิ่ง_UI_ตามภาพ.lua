...
local function listenToPlayerMusic(targetPlayer)
    ...
    if #sounds == 0 then
        return false, "ไม่พบเพลงที่ผู้เล่นกำลังเปิด"
    end
...
MainFrame.Size = UDim2.new(0, 900, 0, 590)
...
local mStroke = stroke(MainFrame, PURPLE, 3, 0.05)
mStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

local mLEDGradient = Instance.new("UIGradient", mStroke)
mLEDGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(55, 0, 100)),
    ColorSequenceKeypoint.new(0.15, Color3.fromRGB(170, 35, 255)),
    ColorSequenceKeypoint.new(0.30, Color3.fromRGB(255, 210, 255)),
    ColorSequenceKeypoint.new(0.50, Color3.fromRGB(135, 20, 255)),
    ColorSequenceKeypoint.new(0.72, Color3.fromRGB(60, 0, 110)),
    ColorSequenceKeypoint.new(0.88, Color3.fromRGB(220, 70, 255)),
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(55, 0, 100))
})
...
task.spawn(function()
    ...
    while MainFrame.Parent do
        ...
        mLEDGradient.Rotation = rotation
        task.wait(0.03)
    end
end)
...
TopBar.Size = UDim2.new(1, -24, 0, 76)
...
Subtitle.Position = UDim2.new(0, 15, 0, 44)
...
LeftPanel.Position = UDim2.new(0, 12, 0, 98)
...
PlayerHeader.Size = UDim2.new(1, -24, 0, 32)
PlayerHeader.Position = UDim2.new(0, 12, 0, 8)
...
ListScroll.Position = UDim2.new(0, 12, 0, 96)
...
-- แผงขวา: ระบบเจาะเพลง/ฟังเพลง
local RightPanel = Instance.new("Frame", MainFrame)
RightPanel.Size = UDim2.new(0.48, -18, 0, 350)
RightPanel.Position = UDim2.new(0.52, 6, 0, 98)
...
MusicHeader.Size = UDim2.new(1, -24, 0, 32)
MusicHeader.Position = UDim2.new(0, 12, 0, 8)
...
MusicHeader.TextSize = 17
MusicHeader.TextXAlignment = Enum.TextXAlignment.Left

local MusicInput = Instance.new("TextBox", RightPanel)
MusicInput.Size = UDim2.new(1, -24, 0, 44)
MusicInput.Position = UDim2.new(0, 12, 0, 48)
MusicInput.BackgroundColor3 = Color3.fromRGB(9, 9, 15)
MusicInput.TextColor3 = WHITE
MusicInput.PlaceholderColor3 = MUTED
MusicInput.PlaceholderText = "ใส่ ID เพลง หรือ ข้อความที่ต้องการเจาะ..."
MusicInput.Text = ""
MusicInput.Font = Enum.Font.Gotham
MusicInput.TextSize = 12
MusicInput.TextXAlignment = Enum.TextXAlignment.Left
MusicInput.ClearTextOnFocus = false
corner(MusicInput, 9)
stroke(MusicInput, Color3.fromRGB(95, 35, 150), 1, 0.1)

local ButtonsContainer = Instance.new("Frame", RightPanel)
ButtonsContainer.Size = UDim2.new(1, -24, 0, 195)
ButtonsContainer.Position = UDim2.new(0, 12, 0, 101)
...
-- หมายเหตุ: ButtonsContainer ยังใช้ชื่อตัวแปรเดิมทั้งหมด
-- เพื่อไม่กระทบกับระบบ Event/ฟังก์ชันด้านล่างของสคริปต์เดิม
...
corner(ListenPanel, 9)

local ListenTitle = Instance.new("TextLabel", ListenPanel)
ListenTitle.Size = UDim2.new(1, -20, 0, 24)
ListenTitle.Position = UDim2.new(0, 10, 0, 4)
...
StatusLabel.Size = UDim2.new(0.52, -18, 0, 52)
StatusLabel.Position = UDim2.new(0, 12, 0, 458)
...
RefreshBtn.Size = UDim2.new(0.48, -18, 0, 52)
RefreshBtn.Position = UDim2.new(0.52, 6, 0, 458)
...
Footer.Size = UDim2.new(1, -24, 0, 52)
Footer.Position = UDim2.new(0, 12, 1, -64)
...
UserInfoLabel.Size = UDim2.new(0.30, 0, 1, 0)
UserInfoLabel.Position = UDim2.new(0, 12, 0, 0)
UserInfoLabel.BackgroundTransparency = 1
UserInfoLabel.Text = "👤  ผู้ใช้: @" .. LocalPlayer.Name
...
SafetyLabel.Text = "🛡  สถานะ: ปลอดภัย"
...
PingLabel.Size = UDim2.new(0.23, -12, 1, 0)
...
ToggleBtn.ZIndex = 10
corner(ToggleBtn, 16)
local tStroke = stroke(ToggleBtn, PURPLE, 2, 0)
setDrag(ToggleBtn, ToggleBtn)

-- ==================== หน้าต่างรองส่อง Real-time ====================
local JunkFrame = Instance.new("Frame", ScreenGui)
JunkFrame.Size = UDim2.new(0, 600, 0, 420)
JunkFrame.Position = UDim2.new(0.5, -300, 0.5, -210)
...
GetIDBtn.MouseButton1Click:Connect(function()
    ...
    StatusLabel.Text = "🔍 กำลังเจาะเสียงจริงจาก Boombox..."

    -- สแกนเฉพาะ Sound ที่กำลังเล่นและอยู่ภายใน Tool ที่ผู้เล่นกำลังถืออยู่
    -- เพื่อไม่ให้ดึงเสียงเดิน/กระโดด หรือ ID อื่นที่ไม่ได้ถูกเปิดผ่าน Boombox
    local realSound = nil
    for _, obj in ipairs(targetPlayer.Character:GetDescendants()) do
        if obj:IsA("Sound") and obj.IsPlaying and obj.SoundId ~= "" then
            local parent = obj.Parent
            local tool = nil
            while parent and parent ~= targetPlayer.Character do
            ...
            if tool then
                ...
                -- รับ Tool ที่ชื่อเกี่ยวกับ boombox/music หรือ Tool ที่มีเสียงเพลงกำลังเล่น
                if string.find(toolName, "boom") or string.find(toolName, "music")
                    or string.find(toolName, "radio") or string.find(soundName, "music")
                    or string.find(soundName, "song") then
                    realSound = obj
                    break
                end
            end
    ...
    if not realSound then
        StatusLabel.Text = "❌ ไม่พบเสียงที่กำลังเล่นผ่าน Boombox"
        return
    end
