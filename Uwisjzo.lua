...
GetIDBtn.MouseButton1Click:Connect(function()
    ...
    if not targetPlayer or not targetPlayer.Character then
        StatusLabel.Text = "❌ ไม่พบผู้เล่นหรือ Character"
        return
    end

    StatusLabel.Text = "🔍 กำลังจับตัวแปร → หา Sound จริงในแมพ..."

    ----------------------------------------------------------------
    -- IMPORTANT:
    -- ปุ่มนี้ใช้ "ตัวแปรที่ระบบเคยจับได้" เป็นตัวอ้างอิงก่อน
    -- แล้วจึงตรวจ Sound ที่มีอยู่จริงในเกม
    --
    -- ไม่ decode junk และไม่ดึงเลขสุ่มจากข้อความ
    ----------------------------------------------------------------
    ...
    local function addCandidate(value)
        ...
        if not candidateMap[id] then
            candidateMap[id] = true
            table.insert(candidateOrder, id)
        end
    end

    -- 1) ตัวแปร source ที่ปุ่มฟังเพลงเคยจับไว้
    if LastHeardMusicSound
        and LastHeardMusicSound:IsA("Sound") then
        addCandidate(LastHeardMusicSound.SoundId)
    end

    addCandidate(LastHeardMusicSoundId)

    -- 2) ดึง Sound candidates จากระบบเดิม
    --    แต่ปุ่มนี้จะไม่บังคับ PlaybackLoudness
    --    เพราะบางเกมเสียงที่เราได้ยินอาจรายงานค่า Loudness เป็น 0
    local oldHeard = getHeardMusicSounds(targetPlayer)

    for _, soundObj in ipairs(oldHeard) do
        if soundObj and soundObj:IsA("Sound") then
            addCandidate(soundObj.SoundId)
        end
    end

    ----------------------------------------------------------------
    -- ถ้ายังไม่มี candidate จากตัวแปร:
    -- ใช้ Sound ที่ "กำลังเล่นจริง" ของผู้เล่นเป็นตัวจับ source
    -- เพื่อให้ปุ่มเจาะไม่ล้มเพราะระบบ loudness ของเกม
    --
    -- ID สุดท้ายยังคงมาจาก Sound.SoundId เท่านั้น
    ----------------------------------------------------------------
    ...
    local function collectSound(obj)
        if not obj
            or not obj:IsA("Sound")
            or not obj:IsDescendantOf(game) then
            return
        end

        if not obj.IsPlaying or obj.SoundId == "" then
            return
        end

        if (tonumber(obj.Volume) or 0) <= 0 then
            return
        end

        if liveSeen[obj] then
            return
        end

        liveSeen[obj] = true
        table.insert(liveSounds, obj)
    end

    -- Character
    do
        local ok, descendants = pcall(function()
            return targetPlayer.Character:GetDescendants()
        end)
    ...
    if preferredRoot and preferredRoot ~= targetPlayer.Character then
        local ok, descendants = pcall(function()
            return preferredRoot:GetDescendants()
        end)
    ...
    if backpack then
        local ok, descendants = pcall(function()
            return backpack:GetDescendants()
        end)
    ...
    -- Workspace:
    -- ไม่ตัดเสียงด้วย PlaybackLoudness
    -- เพราะเราต้องการ Sound ที่กำลังเล่นจริง
    do
        local ok, descendants = pcall(function()
    ...
    ----------------------------------------------------------------
    -- ถ้าระบบเดิมจับ candidate ได้:
    -- ให้ match เฉพาะ Sound ที่ SoundId ตรงกับ candidate
    --
    -- ถ้าไม่มี candidate:
    -- เลือกจาก live Sound ที่ผูกกับ Character/รถของ target
    ----------------------------------------------------------------
    ...
    local function isMovementSound(soundObj)
        local n = string.lower(soundObj.Name)
    ...
    local function scoreSound(soundObj)
        local id = normalizeMusicId(soundObj.SoundId)
        if not id or BlockedIDs[id] then
            return nil
        end

        local score = 0

        -- ตรงกับตัวแปรเดิม = คะแนนสูงมาก
        if candidateMap[id] then
            score += 100000000
        end

        -- อยู่ใน Character = สูง
        if targetPlayer.Character
            and soundObj:IsDescendantOf(targetPlayer.Character) then
            score += 30000000
        end

        -- อยู่ในรถของเป้าหมาย = สูงมาก
        if preferredRoot
            and soundObj:IsDescendantOf(preferredRoot) then
            score += 50000000
        end

        -- ไม่ใช่ชื่อเสียง movement
        if not isMovementSound(soundObj) then
            score += 500000
        else
            score -= 5000000
        end

        -- ถ้าหาตำแหน่งได้ ให้ใช้ระยะเป็นตัวช่วย
        local pos = getSoundWorldPosition(soundObj)

        if origin and pos then
            local distance = (pos - origin).Magnitude

            if distance <= 180 then
                score += math.max(0, 180000 - (distance * 1000))
            else
                score -= 10000000
            end
        else
            -- เสียง 2D อาจไม่มี WorldPosition
            -- อย่าทิ้งมันทันที ถ้ามันอยู่ใน hierarchy ของ target
            if targetPlayer.Character
                and soundObj:IsDescendantOf(targetPlayer.Character) then
                score += 1000000
            elseif preferredRoot
                and soundObj:IsDescendantOf(preferredRoot) then
                score += 1000000
            end
        end

        -- Loudness เป็น "คะแนนเสริม" เท่านั้น
        -- ไม่ใช่เงื่อนไขตัดทิ้ง
        local loudness = tonumber(soundObj.PlaybackLoudness) or 0
        score += math.min(loudness, 1000) * 100

        return score, id
    end

    for _, soundObj in ipairs(liveSounds) do
        local score = scoreSound(soundObj)

        if score then
            local numericScore = score

            if numericScore > bestScore then
    ...
    ----------------------------------------------------------------
    -- ถ้ามี candidate จากตัวแปร ต้องบังคับให้ผลสุดท้ายตรงกับ
    -- candidate นั้นจริง ๆ
    ----------------------------------------------------------------
    ...
    if next(candidateMap) ~= nil then
        ...
        for _, soundObj in ipairs(liveSounds) do
            ...
            if id and candidateMap[id] then
                local score = 0
                ...
                if score > matchedScore then
                    matchedScore = score
                    matchedSound = soundObj
                end
            end
        end

        bestSound = matchedSound
    end

    ----------------------------------------------------------------
    -- ไม่มี Sound ที่ตรงกับตัวแปรและกำลังเล่นจริง
    ----------------------------------------------------------------

    if not bestSound then
        if next(candidateMap) ~= nil then
            StatusLabel.Text =
                "❌ ตัวแปรจับ ID ได้ แต่ยังหา Sound ID เดียวกันที่กำลังเล่นในแมพไม่เจอ"
        else
            StatusLabel.Text =
                "❌ ไม่พบ Sound ที่กำลังเล่นจริงของผู้เล่น"
        end
        return
    end
    ...
    if not bestSound:IsDescendantOf(game)
        or not bestSound.IsPlaying
        or bestSound.SoundId == "" then
        StatusLabel.Text = "❌ เพลงหยุด/เปลี่ยนก่อนเจาะเสร็จ"
        return
    end

    if (tonumber(bestSound.Volume) or 0) <= 0 then
        StatusLabel.Text = "❌ Sound ไม่มี Volume ที่กำลังเล่น"
        return
    end

    local finalId = normalizeMusicId(bestSound.SoundId)

    if not finalId or finalId == "" then
        StatusLabel.Text = "❌ Sound จริงไม่มี Asset ID ที่อ่านได้"
        return
    end

    -- ถ้ามีตัวแปร candidate ต้องตรงกัน
    if next(candidateMap) ~= nil and not candidateMap[finalId] then
        StatusLabel.Text = "❌ ID ของ Sound จริงไม่ตรงกับตัวแปรที่จับไว้"
        return
    end
    ...
    ----------------------------------------------------------------
    -- สำเร็จ: ได้เพียง 1 ID
    ----------------------------------------------------------------

    copyToClipboard(finalId)

    StatusLabel.Text =
        "📋 เจาะเพลงจริงสำเร็จ 1 ID: " .. finalId
end)
