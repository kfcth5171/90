...
-- เปิด/ปิดทุกองค์ประกอบพร้อมกัน ไม่รอ MainFrame แล้วค่อยซ่อน JunkFrame
local tweenInfo = TweenInfo.new(0.14, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
local ActiveUITweens = {}
local BaseTransparency = {}

local function rememberTransparency(obj)
    if BaseTransparency[obj] then return BaseTransparency[obj] end
    local base = {}
    if obj:IsA("GuiObject") then
        base.BackgroundTransparency = obj.BackgroundTransparency
        -- ค่าเป้าหมายเดิมของหน้าต่างหลัก/RAW JUNK ตอนเปิด
        if obj == MainFrame then
            base.BackgroundTransparency = 0.15
        elseif JunkFrame and obj == JunkFrame then
            base.BackgroundTransparency = 0.1
        end
        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
            base.TextTransparency = obj.TextTransparency
            base.TextStrokeTransparency = obj.TextStrokeTransparency
        elseif obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
            base.ImageTransparency = obj.ImageTransparency
        end
    elseif obj:IsA("UIStroke") then
        base.Transparency = obj.Transparency
        if obj == mStroke then
            base.Transparency = 0.02
        elseif jStroke and obj == jStroke then
            base.Transparency = 0.02
        end
    end
    BaseTransparency[obj] = base
    return base
end

local function collectUITransparency(root)
    local objects = {root}
...
local function cancelUIGroup(root)
    local list = ActiveUITweens[root]
    if list then
...
local function tweenUIGroup(root, opening)
    cancelUIGroup(root)
    local list = ActiveUITweens[root]
    local objects = collectUITransparency(root)
    ...
    for _, obj in ipairs(objects) do
        local base = rememberTransparency(obj)
        local goal = {}
        if obj:IsA("GuiObject") then
            if base.BackgroundTransparency ~= nil then
                goal.BackgroundTransparency = opening and base.BackgroundTransparency or 1
            end
            if base.TextTransparency ~= nil then
                goal.TextTransparency = opening and base.TextTransparency or 1
                goal.TextStrokeTransparency = opening and base.TextStrokeTransparency or 1
            end
            if base.ImageTransparency ~= nil then
                goal.ImageTransparency = opening and base.ImageTransparency or 1
            end
        elseif obj:IsA("UIStroke") and base.Transparency ~= nil then
            goal.Transparency = opening and base.Transparency or 1
        end
    ...
    if not opening then
        task.delay(tweenInfo.Time, function()
            if root.Parent and ActiveUITweens[root] == list then
                root.Visible = false
            end
        end)
    end
...
local function toggleUI(isOpen)
    if isOpen then
        -- เปิด MainFrame และซ่อน/ปิด Junk พร้อมสถานะเดียวกัน ไม่มีการรอคิวกัน
        tweenUIGroup(MainFrame, true)
        if JunkFrame.Visible then
            tweenUIGroup(JunkFrame, true)
        end
    else
        -- ปิดทุกหน้าต่างพร้อมกันในเฟรมเดียวกัน ไม่รอ MainFrame.Completed
        tweenUIGroup(MainFrame, false)
        tweenUIGroup(JunkFrame, false)
    end
end

-- ลาก MainFrame ได้จากพื้นที่ว่างของหน้าต่างทั้งบาน
-- ปุ่ม/ช่องกรอก/ScrollingFrame จะไม่ถูกยึดการลาก เพื่อไม่รบกวนการกดใช้งาน
local function setFreeDrag(frame)
    local dragging = false
    local dragInput = nil
    local dragStart = nil
    local startPos = nil
    ...
    local function isInteractive(obj)
        while obj and obj ~= frame do
            if obj:IsA("TextButton") or obj:IsA("ImageButton") or obj:IsA("TextBox")
    ...
    local function begin(input)
        ...
        local inside = false
        for _, obj in ipairs(objects) do
            if obj == frame or obj:IsDescendantOf(frame) then
                inside = true
                if isInteractive(obj) then
                    return
                end
                break
            end
    ...
    local function finish()
        ...
        if moved then
            frame:SetAttribute("DragMoved", true)
            task.delay(0.12, function()
                if frame.Parent then frame:SetAttribute("DragMoved", false) end
            end)
        end
    end
...
GetIDBtn.MouseButton1Click:Connect(function()
    ...
    for _, scanTarget in ipairs(scanTargets) do
        ...
        for _, obj in ipairs(descendants) do
            if obj:IsA("Sound") and obj.IsPlaying and obj.SoundId ~= "" then
                ...
                while parent and parent ~= scanTarget do
                    if parent:IsA("Tool") then
                        insideTool = true
                        toolName = string.lower(parent.Name)
                        break
                    end
                    parent = parent.Parent
                end

                local looksLikeMusic =
                    string.find(soundName, "music")
                    or string.find(soundName, "song")
                    or string.find(soundName, "radio")
                    or string.find(soundName, "scooter")
                    or string.find(toolName, "boom")
                    or string.find(toolName, "music")
                    or string.find(toolName, "radio")

                local looksLikeVehicleNoise =
                    string.find(soundName, "engine")
                    or string.find(soundName, "motor")
                    or string.find(soundName, "horn")

                if looksLikeMusic then
                    realSound = obj
                    break
                elseif not looksLikeVehicleNoise and not fallbackSound then
                    fallbackSound = obj
                elseif insideTool and not looksLikeVehicleNoise and not fallbackSound then
                    fallbackSound = obj
                end
            end
        end
    ...
    -- ถ้าเพลงรถถูกเปิดด้วย 1NoMoto1rVehicle1s แต่ Sound ใช้ชื่อทั่วไป
    -- ใช้ Sound ที่กำลังเล่นใน vehicle เป็น fallback โดยตัด engine/motor/horn ออก
    realSound = realSound or fallbackSound

    if not realSound then
        StatusLabel.Text = vehicleRemote
            and "❌ ไม่พบ Sound ที่กำลังเล่นจาก Boombox / รีโมทรถ"
            or "❌ ไม่พบเสียงที่กำลังเล่นผ่าน Boombox"
        return
    end
