...
local function extractIDsFromPattern(text)
    ...
    for _, pat in ipairs(patterns) do
        for capture in string.gmatch(text, pat) do
            -- ค่า variable อาจมีข้อความห่ออยู่ จึงดึงเฉพาะเลขจากค่าของ variable นี้เท่านั้น
            for num in string.gmatch(capture, "%d+") do
                if not BlockedIDs[num] and not seen[num] then
                    seen[num] = true
...
-- ==================== ยืนยัน ID จากเสียงจริง + ตัวแปร ====================
-- ยึด Sound ที่กำลังเล่นจริงของผู้เล่นก่อน แล้วตรวจว่าค่าที่ตัวแปรเจาะได้ตรงกับ ID ของ Sound นั้น
...
local function getVerifiedPlayerMusic(targetPlayer)
    ...
    for _, soundObj in ipairs(sounds) do
        -- ชั้นแรก: ต้องเป็น Sound ที่ระบบหาได้ว่าเล่น/ได้ยินจริงและผูกกับผู้เล่นอยู่แล้ว
        if soundObj and soundObj.Parent and soundObj.IsPlaying
            and soundObj.SoundId ~= ""
            and (tonumber(soundObj.PlaybackLoudness) or 0) > 0.1 then
            ...
            -- ถ้า SoundId เป็น asset ID ตรง ๆ นี่คือ ID ของเสียงที่เล่นจริงโดยตรง
            if realId and not BlockedIDs[realId] then
                -- ถ้ามีตัวแปรเพลงอยู่ ต้องมีค่าตรงกับเสียงจริง จึงจะผ่านการยืนยัน
                if #variableIds == 0 then
                    return soundObj, realId, "DIRECT_REAL_SOUND"
                end
                for _, variableId in ipairs(variableIds) do
                    if variableId == realId then
                        return soundObj, realId, "VARIABLE_MATCHED_REAL_SOUND"
                    end
                end
                -- ตัวแปรไม่ตรงกับเสียงจริง: ข้ามทันที ไม่ดึง ID หลอก
            else
                -- SoundId ไม่ได้เป็น ID ตรง ๆ: รับเฉพาะตัวแปรที่ให้ ID เดียวแบบไม่กำกวม
                if #variableIds == 1 then
                    return soundObj, variableIds[1], "SINGLE_VARIABLE_FROM_REAL_SOUND"
                end
            end
        end
    end
